...
from foundry.finance.resilience_assessment import (
    ...
    TARGET_METRIC as RESILIENCE_TARGET_METRIC,
)
from foundry.finance.resilience_evidence import record_resilience_evidence
...
def build(log: EventLog, as_of: float | None = None) -> MorganHousehold:
    ...
    resilience_assumptions = fin.declare_assumption_set(
        log, "Financial Resilience policy", "v1", {
    ...
    fin.declare_scenario(
        log, "Maintain a £250 monthly emergency-reserve contribution",
        resilience_assumptions.id,
        ...
        actor="synthetic_demo")
    declare_mission(
        log, "Financial Resilience — 18 months and protections",
        target_metric=RESILIENCE_TARGET_METRIC,
        target_value=18.0,
    ...
    resilience_common = {
        ...
        "lineage": (
            "Synthetic RFC-008 proof data generated by "
            "foundry.demo_data.build"),
        "unit_or_currency": "GBP",
        "actor": "synthetic_demo",
    }
    for index, amount in enumerate((5_400.0, 4_200.0), start=1):
        record_resilience_evidence(
            ...
            **{
                **resilience_common,
                "source": f"synthetic declared income source {index}",
            },
            description=f"synthetic declared income source {index}",
        )
    record_resilience_evidence(
        log,
        hh.household_id,
        ...
        as_of,
        **resilience_common,
        due_at=as_of + 180 * DAY,
        description="synthetic annual household commitment",
    )
    ...
    # observations distinct from the pension-account contribution ledger.
    fin.declare_valuation(
        log, hh.alex_pension_id, 34_000.0, "GBP", as_of - 180 * DAY,
        actor="synthetic_demo")
    fin.declare_valuation(
        log, hh.sam_pension_id, 21_000.0, "GBP", as_of - 180 * DAY,
        actor="synthetic_demo")
    fin.declare_valuation(
        log, hh.alex_pension_id, 38_000.0, "GBP", as_of,
        actor="synthetic_demo")
    fin.declare_valuation(
        log, hh.sam_pension_id, 24_000.0, "GBP", as_of,
        actor="synthetic_demo")

    pension_assumptions = fin.declare_assumption_set(
        log, "Pension Independence real-terms sensitivities", "v1", {
            ...
            "accelerated_threshold_months": 36.0,
            "divergent_floor_fraction": .75,
            "milestone_fraction_1": .25,
            "milestone_fraction_2": .50,
            "milestone_fraction_3": .75,
            "milestone_fraction_4": 1.0,
            "surplus_high_fraction": .20,
            "shortfall_low_fraction": .10,
            "sp_reliance_low_fraction": 1 / 3,
            "sp_reliance_mid_fraction": .50,
            "sp_reliance_high_fraction": 2 / 3,
            "delta_v_lookback_days": 90.0,
            "recommendation_liquidity_floor_months": 6.0,
        }, actor="synthetic_demo")
    fin.declare_scenario(
        log, "Test a £250 monthly pension contribution increase",
        pension_assumptions.id,
        {"monthly_pension_contribution_delta": 250.0},
        action_type="increase_pension_contribution",
        action_label="Increase pension contribution",
        unit_or_currency="GBP",
        cadence="month",
        actor="synthetic_demo")
    declare_mission(
        log,
        "Pension Independence at State Pension age",
    ...
    pension_common = {
        "confidence": .9,
        "source": "synthetic pension statement",
        "lineage": (
            "Synthetic RFC-009 proof data generated by "
            "foundry.demo_data.build; no policy or account identifier"),
        "actor": "synthetic_demo",
    }

    def pension_evidence(
        subject_id: str,
        field: str,
        value,
        *,
        unit_or_currency: str | None,
        effective_at: float = as_of - 200 * DAY,
    ) -> None:
    ...
    for account_id, employee, employer, sacrifice, fee in (
        (hh.alex_pension_id, 6_065.17, 3_500.0, 800.0, .006),
        (hh.sam_pension_id, 3_000.0, 2_500.0, 1_000.0, .007),
    ):
        pension_evidence(
            account_id, "employee_contribution_annual", employee,
            unit_or_currency="GBP")
        pension_evidence(
            account_id, "employer_contribution_annual", employer,
            unit_or_currency="GBP")
        pension_evidence(
            account_id, "salary_sacrifice_annual", sacrifice,
            unit_or_currency="GBP")
        pension_evidence(
            account_id, "annual_fee_percent", fee,
            unit_or_currency="fraction")

    for subject_id, annual, age in (
        (hh.alex_id, 6_000.0, 67.0),
        (hh.sam_id, 4_600.0, 68.0),
    ):
        pension_evidence(
            subject_id, "state_pension_annual", annual,
            unit_or_currency="GBP")
        pension_evidence(
            subject_id, "state_pension_age", age,
            unit_or_currency="years")
        pension_evidence(
            subject_id, "state_pension_basis", "accrued_to_date",
            unit_or_currency=None)

    for account_id, field, amount in (
        (
            hh.alex_pension_id,
        ...
        pension_evidence(
            ...
            amount,
            unit_or_currency="GBP",
            effective_at=as_of - 20 * DAY,
        )

    current_tax_year = fin.declare_tax_jurisdiction(
        log,
        "UK-CURRENT-SYNTHETIC",
        tax_year_start=as_of - 100 * DAY,
        tax_year_end=as_of + 265 * DAY,
        actor="synthetic_demo",
    )
    for member_id in (
        hh.alex_id,
        ...
        fin.tax_resident_in(
            log, member_id, current_tax_year.id, actor="synthetic_demo")
